document.getElementById('search').addEventListener('input', async function() {
    ...
    // Clear the container when typing
    fileContainer.innerHTML = searchTerm ? '' : '<p class="has-text-black">Start typing to search files.</p>';
    ...
    try {
        // Fetch files with a search query
        const response = await fetch(`/api/search-subject-files?query=${encodeURIComponent(searchTerm)}&folder=${encodeURIComponent(folderName)}`);
        ...
        files.forEach((file) => {
            ...
            fileElement.download = fileNameWithExtension;
